<?php
class excitor_Social_Widget extends WP_Widget {
    function __construct() {
        parent::__construct(
                'social_widget', // Base ID
                esc_html__('Socials', 'excitor'), // Name
                array('description' => esc_html__('Social Widget', 'excitor'),) // Args
        );
    }
    ...
    function form($instance) {
		...
		?>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Title', 'excitor'); ?></label>
        ...
        for ($i = 1; $i <= 12; $i++) {
            ?>
            <p>
                <label for="<?php echo esc_url($this->get_field_id('icon_social_' . $i)); ?>"><?php echo esc_html__('Social Icon', 'excitor').$i; ?></label>
            ...
            <p>
                <label for="<?php echo esc_url($this->get_field_id('link_social_' . $i)); ?>"><?php echo esc_html__('Social Link', 'excitor').$i; ?></label>
        ...
        <?php } ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('el_class')); ?>"><?php _e('Extra Class', 'excitor'); ?></label>
...
/**
 * Class excitor_Social_Widget
 */
function excitor_register_social_widget() {
    register_widget('excitor_Social_Widget');
}
add_action('widgets_init', 'excitor_register_social_widget');
?>
