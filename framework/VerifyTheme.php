  ...
  class BearsthemesCommunicator{
    ...
    function registerDomain($purchaseCode, $domain) {
        ...
        if ( is_wp_error( $response ) ) {
           ...
           $this->message = __("Something went wrong on unregister your domain: ", "excitor").$error_message;
           return null;
        } else {
           return true;
        }
    ...
    function getConnectedDomains($purchaseCode) {
      ...
      if ( is_wp_error( $response ) ) {
         $error_message = $response->get_error_message();
         $this->message = __("Something went wrong on get connected domain with your purchase code: ", "excitor").$error_message;
         return null;
      } else {
    ...
    function getPurchaseInformation($purchaseCode) {
      ...
      if ( is_wp_error( $response ) ) {
         $error_message = $response->get_error_message();
         $this->message = __("Something went wrong on get purchase information: ", "excitor").$error_message;
         return null;
      } else {
         $result = json_decode($response['body']);
         return $result;
    ...
    public function verifytheme_sanitize( $input )
    {
        ...
        if ($ok_purchase_code) {
          ...
          if($communicator->message){
            ...
            $message .= __("Invalid purchase code<br />","excitor");
          }
        }
