...
/** @internal */
function excitor_filter_theme_ext_mega_menu_wp_nav_menu_args($args) {
	$args['walker'] = new FW_Ext_Mega_Menu_Custom_Walker();

	return $args;
}
add_filter('wp_nav_menu_args', 'excitor_filter_theme_ext_mega_menu_wp_nav_menu_args');

if(!function_exists('excitor_menu_item_dynamic_sidebar')) {
	function excitor_menu_item_dynamic_sidebar( $content = '', $item ) {
		// extra options
		$extra_options = array();
		$sidebar_content = '';
...
add_filter( 'walker_nav_menu_start_el_custom', 'excitor_menu_item_dynamic_sidebar', 10, 2 );
